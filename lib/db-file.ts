import 'server-only';
import fs from 'fs';
import path from 'path';
import type { BakeOfWeek, MenuItem, Order, Testimonial, Setting, CakeMenuItemRecord } from '@/types';

const DATA_DIR = path.join(process.cwd(), 'data');
const DB_FILE = path.join(DATA_DIR, 'db.json');
const TMP_FILE = `${DB_FILE}.${process.pid}.tmp`;

interface DatabaseSchema {
  products: MenuItem[];
  orders: Order[];
  testimonials: Testimonial[];
  settings: Setting[];
  bake_of_week: BakeOfWeek[];
  cake_menu_items: CakeMenuItemRecord[];
}

const defaultProducts: MenuItem[] = [
  {
    id: 'c1',
    name: 'Sunflower Cream Cake',
    description: 'A light layered sponge kissed with organic cream and sunflower honey, decorated with seasonal blooms.',
    price: 1800,
    unit: '/whole',
    category: 'cake',
    badge: 'Fresh Today',
    image: 'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=600&h=500&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1571115177098-24ec42ed204d?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1565958011703-44f9829ba187?w=600&h=500&fit=crop',
    ],
    rating: 5,
    featured: true,
    is_bake_of_week: true,
    display_order: 1,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'c2',
    name: 'Chocolate Hazelnut Tart',
    description: 'Rich dark chocolate ganache poured into a hand-pressed pastry shell, crowned with roasted hazelnuts.',
    price: 950,
    unit: '/piece',
    category: 'pastry',
    badge: 'Organic',
    image: 'https://images.unsplash.com/photo-1519915028121-7d3463d20b13?w=600&h=500&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1519915028121-7d3463d20b13?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1606313564200-e75d5e30476c?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1624353365286-3f8d62daad51?w=600&h=500&fit=crop',
    ],
    rating: 5,
    featured: true,
    display_order: 2,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'c3',
    name: 'Heritage Sourdough Loaf',
    description: '72-hour cold-fermented sourdough made with heritage wheat and a century-old starter culture.',
    price: 650,
    unit: '/loaf',
    category: 'bread',
    badge: 'Organic',
    image: 'https://images.unsplash.com/photo-1509440159596-0249088772ff?w=600&h=500&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1509440159596-0249088772ff?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1549931319-a545dcf3bc73?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1586444248902-2f64eddc13df?w=600&h=500&fit=crop',
    ],
    rating: 5,
    featured: true,
    display_order: 3,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'c4',
    name: 'Butter Croissant',
    description: 'Flaky golden layers of French-style buttery pastry, hand-laminated and baked fresh each morning.',
    price: 180,
    unit: '/piece',
    category: 'pastry',
    badge: 'Fresh Today',
    image: 'https://images.unsplash.com/photo-1623334044303-241021148842?w=600&h=500&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1623334044303-241021148842?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1555507036-ab1f4038808a?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1555507036-ab1f4038808a?w=600&h=500&fit=crop',
    ],
    rating: 5,
    featured: false,
    display_order: 4,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'c5',
    name: 'Strawberry Tart',
    description: 'Crisp pastry shell filled with vanilla custard and topped with fresh Himalayan strawberries.',
    price: 650,
    unit: '/piece',
    category: 'pastry',
    badge: 'Seasonal',
    image: 'https://images.unsplash.com/photo-1558961363-fa8fdf82db35?w=600&h=500&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1558961363-fa8fdf82db35?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1464305795204-6f5bbfc7fb81?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1488477181946-6428a0291777?w=600&h=500&fit=crop',
    ],
    rating: 4,
    featured: false,
    display_order: 5,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'c6',
    name: 'Cinnamon Roll',
    description: 'Soft dough rolled with Ceylon cinnamon and topped with tangy cream cheese glaze.',
    price: 220,
    unit: '/piece',
    category: 'pastry',
    badge: 'Popular',
    image: 'https://images.unsplash.com/photo-1509440159596-0249088772ff?w=600&h=500&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1509440159596-0249088772ff?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1567958451986-2de427a4a0be?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1517433670267-08bbd4be890f?w=600&h=500&fit=crop',
    ],
    rating: 5,
    featured: false,
    display_order: 6,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'c7',
    name: 'Rustic Country Loaf',
    description: 'Stone-ground whole wheat with a deep caramelized crust and nutty interior crumb.',
    price: 480,
    unit: '/loaf',
    category: 'bread',
    badge: 'Organic',
    image: 'https://images.unsplash.com/photo-1586985289688-ca3cf47d3e6e?w=600&h=500&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1586985289688-ca3cf47d3e6e?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1555507036-ab1f4038808a?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1509440159596-0249088772ff?w=600&h=500&fit=crop',
    ],
    rating: 5,
    featured: false,
    display_order: 7,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'c8',
    name: 'Chocolate Hazelnut Cake',
    description: 'Dark Belgian chocolate layered with roasted hazelnut praline and velvety ganache.',
    price: 2200,
    unit: '/whole',
    category: 'cake',
    badge: 'Best Seller',
    image: 'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=600&h=500&fit=crop',
    images: [
      'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1571115177098-24ec42ed204d?w=600&h=500&fit=crop',
      'https://images.unsplash.com/photo-1542826438-bd32f43d626f?w=600&h=500&fit=crop',
    ],
    rating: 5,
    featured: true,
    display_order: 8,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
];

const defaultTestimonials: Testimonial[] = [
  {
    id: 't1',
    name: 'Anita Shrestha',
    initials: 'AS',
    role: 'Regular Customer',
    text: 'The sourdough here is on another level. I drive 30 minutes just for their bread. The crust, the flavor, the texture — absolute perfection every single time.',
    rating: 5,
    approved: true,
    created_at: new Date(Date.now() - 86400000 * 3).toISOString(),
  },
  {
    id: 't2',
    name: 'Rajesh Patel',
    initials: 'RP',
    role: 'Birthday Order',
    text: 'Ordered a custom birthday cake and it was stunning. Not only did it look beautiful, but every slice was moist and delicious. Everyone at the party was amazed.',
    rating: 5,
    approved: true,
    created_at: new Date(Date.now() - 86400000 * 5).toISOString(),
  },
  {
    id: 't3',
    name: 'Sunita Gurung',
    initials: 'SG',
    role: 'Food Blogger',
    text: 'Their croissants remind me of the ones I had in Paris. Flaky, buttery, and absolutely heavenly. Jiri Bakes has become my weekend ritual.',
    rating: 5,
    approved: true,
    created_at: new Date(Date.now() - 86400000 * 8).toISOString(),
  },
];

const defaultOrders: Order[] = [
  {
    id: 'ord-101',
    customer_name: 'Pooja Sharma',
    customer_phone: '+977 9841234567',
    customer_email: 'pooja.sharma@example.com',
    customer_address: 'Lokanthali, Bhaktapur',
    items: [
      { product_id: 'c1', name: 'Sunflower Cream Cake', price: 1800, quantity: 1 },
      { product_id: 'c4', name: 'Butter Croissant', price: 180, quantity: 4 },
    ],
    total: 2520,
    status: 'completed',
    notes: 'Please add birthday candles and write Happy Birthday Pooja.',
    created_at: new Date(Date.now() - 3600000 * 4).toISOString(),
    updated_at: new Date(Date.now() - 3600000 * 2).toISOString(),
  },
  {
    id: 'ord-102',
    customer_name: 'Bikram Thapa',
    customer_phone: '+977 9801987654',
    customer_email: 'bikram.thapa@example.com',
    customer_address: 'Koteshwor, Kathmandu',
    items: [
      { product_id: 'c3', name: 'Heritage Sourdough Loaf', price: 650, quantity: 2 },
      { product_id: 'c2', name: 'Chocolate Hazelnut Tart', price: 950, quantity: 2 },
    ],
    total: 3200,
    status: 'baking',
    notes: 'Please slice the sourdough loaf.',
    created_at: new Date(Date.now() - 3600000 * 1).toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'ord-103',
    customer_name: 'Srijana KC',
    customer_phone: '+977 9860112233',
    customer_email: 'srijana.kc@example.com',
    customer_address: 'Thimi, Bhaktapur',
    items: [
      { product_id: 'c8', name: 'Chocolate Hazelnut Cake', price: 2200, quantity: 1 },
    ],
    total: 2200,
    status: 'pending',
    notes: 'Delivery by 4 PM today please.',
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
];

const defaultSettings: Setting[] = [
  { key: 'site_name', value: 'Jiri Bakes' },
  { key: 'tagline', value: 'Simply Organic' },
  { key: 'site_title', value: 'Jiri Bakes – Simply Organic Artisan Bakery' },
  { key: 'site_tagline', value: 'Where Flour Meets Feeling' },
  { key: 'phone', value: '+977 1-4567890' },
  { key: 'whatsapp_phone', value: '9779841234567' },
  { key: 'email', value: 'hello@jiribakes.com.np' },
  { key: 'address', value: 'Lokanthali, Araniko Highway, Bhaktapur, Nepal' },
  { key: 'hours_weekday', value: 'Mon – Sat: 7:00 AM – 8:00 PM' },
  { key: 'hours_sunday', value: 'Sunday: 8:00 AM – 6:00 PM' },
  { key: 'fresh_bread_time', value: '7:30 AM' },
  { key: 'delivery_fee', value: '100' },
  { key: 'currency_symbol', value: 'NPR' },
  { key: 'hero_headline', value: 'Baked Like Art.' },
  { key: 'hero_subheadline', value: 'Handcrafted organic breads, cakes, and morning pastries. Baked fresh daily at dawn in Lokanthali, Nepal.' },
  { key: 'announcement_banner', value: 'Fresh organic sourdough available every morning at 7:30 AM!' },
  { key: 'we_care_image', value: '/we care.png' },
];

// â”€â”€â”€ Concurrency-safe file access â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// All DB operations are serialized through this in-process queue so that two
// simultaneous requests (e.g. a customer placing an order while an admin
// updates a product) can never interleave read-modify-write cycles.
let queue: Promise<unknown> = Promise.resolve();

function enqueue<T>(task: () => T | Promise<T>): Promise<T> {
  const run = queue.then(task);
  queue = run.then(() => undefined, () => undefined);
  return run;
}

function sleepSync(ms: number) {
  Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, ms);
}

export const defaultCakeMenuItems: CakeMenuItemRecord[] = [
  {
    id: 'cake-1',
    name: 'Classic Vanilla',
    price: 550,
    cat: 'popular',
    badge: 'Popular',
    desc: 'Light golden sponge layered with pure Madagascar vanilla whipped cream and white chocolate shavings.',
    tags: ['Classic', 'Vanilla', 'Whipped Cream'],
    image: 'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 1,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-2',
    name: 'Black Forest',
    price: 650,
    cat: 'popular',
    badge: 'Classic',
    desc: 'Layers of cocoa-rich chocolate sponge, sour cherries, whipped cream and dark chocolate curls.',
    tags: ['Chocolate', 'Cherries', 'German Classic'],
    image: 'https://images.unsplash.com/photo-1606313564200-e75d5e30476c?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 2,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-3',
    name: 'Strawberry / Blueberry',
    price: 700,
    cat: 'popular',
    badge: 'Berry Delight',
    desc: 'Fluffy vanilla sponge infused with fresh mountain berry coulis and seasonal compote.',
    tags: ['Berries', 'Fruity', 'Seasonal'],
    image: 'https://images.unsplash.com/photo-1565958011703-44f9829ba187?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 3,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-4',
    name: 'Pineapple / Mango',
    price: 700,
    cat: 'popular',
    badge: 'Tropical',
    desc: 'Tropical fruit gateau soaked with fresh pineapple glaze and sun-ripened mango essence.',
    tags: ['Tropical', 'Fresh Fruit', 'Pineapple'],
    image: 'https://images.unsplash.com/photo-1535141192574-5d4897c13136?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 4,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-5',
    name: 'White Forest',
    price: 700,
    cat: 'popular',
    badge: 'Pure Vanilla',
    desc: 'Snow-white vanilla sponge frosted with whipped Chantilly cream and ivory chocolate flakes.',
    tags: ['White Chocolate', 'Whipped Cream'],
    image: 'https://images.unsplash.com/photo-1588195538326-c5b1e9f80a1b?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 5,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-6',
    name: 'Choco Vanilla',
    price: 750,
    cat: 'popular',
    badge: 'Best Duo',
    desc: 'The best of both worlds: marbled chocolate and vanilla sponges with velvety frosting.',
    tags: ['Marble', 'Chocolate', 'Vanilla'],
    image: 'https://images.unsplash.com/photo-1571115177098-24ec42ed204d?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 6,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-7',
    name: 'Choco Mocca',
    price: 800,
    cat: 'popular',
    badge: 'Coffee Rich',
    desc: 'Espresso-soaked chocolate sponge with creamy mocha buttercream and cocoa dust.',
    tags: ['Coffee', 'Mocha', 'Espresso'],
    image: 'https://images.unsplash.com/photo-1586985289688-ca3cf47d3e6e?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 7,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-8',
    name: 'Chocolate Chip Cake',
    price: 800,
    cat: 'popular',
    badge: 'Choc Lovers',
    desc: 'Moist chocolate sponge packed with Belgian chocolate chips and glossy fudge glaze.',
    tags: ['Choco Chips', 'Belgian', 'Fudge'],
    image: 'https://images.unsplash.com/photo-1541781774459-bb2af2f05b55?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 8,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-9',
    name: 'Butterscotch',
    price: 800,
    cat: 'popular',
    badge: 'Crunchy',
    desc: 'Caramel-infused butter sponge layered with crunchy butterscotch praline and butter glaze.',
    tags: ['Caramel', 'Praline', 'Crunch'],
    image: 'https://images.unsplash.com/photo-1542826438-bd32f43d626f?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 9,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-10',
    name: 'Chocolate Truffle Fudge',
    price: 900,
    cat: 'popular',
    badge: 'Decadent',
    desc: 'Dense, decadent chocolate sponge slathered in smooth melted fudge and truffle ganache.',
    tags: ['Truffle', 'Fudge Ganache', 'Rich'],
    image: 'https://images.unsplash.com/photo-1624353365286-3f8d62daad51?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 10,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-11',
    name: 'Cassatto Cake',
    price: 900,
    cat: 'special',
    badge: "Chef's Choice",
    desc: 'Traditional Sicilian-style party cake with rich ricotta cream, candied fruits, and marzipan rim.',
    tags: ['Italian', 'Ricotta', 'Candied Fruit'],
    image: 'https://images.unsplash.com/photo-1535141192574-5d4897c13136?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 11,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-12',
    name: 'Oreo Chocolate',
    price: 900,
    cat: 'popular',
    badge: 'Kids Favourite',
    desc: 'Dark cocoa cake filled and topped with crushed Oreo cookies and sweet cookies & cream frosting.',
    tags: ['Oreo', 'Cookies & Cream'],
    image: 'https://images.unsplash.com/photo-1563729784474-d77dbb933a9e?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 12,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-13',
    name: 'No-Bake Chilled Cake',
    price: 1000,
    cat: 'cheesecake',
    badge: 'Chilled',
    desc: 'Cool, silky smooth set cream cheese on a crunchy honey biscuit crust, served chilled.',
    tags: ['No-Bake', 'Chilled', 'Cream Cheese'],
    image: 'https://images.unsplash.com/photo-1533134242443-d4fd215305ad?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 13,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-14',
    name: 'Red Velvet',
    price: 1200,
    cat: 'special',
    badge: 'Signature',
    desc: 'Vibrant crimson buttermilk sponge with a hint of cocoa, filled with silky cream cheese frosting.',
    tags: ['Red Velvet', 'Cream Cheese', 'Signature'],
    image: 'https://images.unsplash.com/photo-1586788680434-30d324b2d46f?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 14,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-15',
    name: 'Khuwa Cake',
    nepali_subtitle: 'Signature Himalayan Specialty',
    price: 1400,
    cat: 'special',
    badge: 'Local Special',
    desc: 'Signature Himalayan specialty baked with slow-reduced Jiri cow milk mawa and saffron cardamom.',
    tags: ['Khuwa', 'Jiri Milk', 'Himalayan', 'Saffron'],
    image: 'https://images.unsplash.com/photo-1509440159596-0249088772ff?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 15,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-16',
    name: 'Choco Truffle',
    price: 1400,
    cat: 'special',
    badge: 'Premium Dark',
    desc: 'Pure luxury: dense French dark chocolate sponge layered with 70% dark ganache.',
    tags: ['70% Dark', 'French Ganache', 'Luxury'],
    image: 'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 16,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-17',
    name: 'NY Cheese Cake',
    price: 1800,
    cat: 'cheesecake',
    badge: 'Baked NYC',
    desc: 'Authentic slow-baked dense New York style cheesecake made with rich cream cheese and vanilla.',
    tags: ['Baked', 'New York Style', 'Creamy'],
    image: 'https://images.unsplash.com/photo-1533134242443-d4fd215305ad?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 17,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
  {
    id: 'cake-18',
    name: 'Basque Burnt Cheesecake',
    price: 2500,
    cat: 'cheesecake',
    badge: 'San Sebastian',
    desc: 'San Sebastian style caramelized burnt crust with an irresistibly creamy, melting molten custard center.',
    tags: ['Basque', 'Burnt Top', 'Molten Cream'],
    image: 'https://images.unsplash.com/photo-1524351199678-941a58a3df50?w=600&h=500&fit=crop',
    weight: '1 Pound',
    eggless: true,
    available: true,
    display_order: 18,
    created_at: new Date().toISOString(),
    updated_at: new Date().toISOString(),
  },
];

function normalise(parsed: Partial<DatabaseSchema> | null): DatabaseSchema {
  return {
    products: Array.isArray(parsed?.products) ? parsed!.products! : defaultProducts,
    orders: Array.isArray(parsed?.orders) ? parsed!.orders! : defaultOrders,
    testimonials: Array.isArray(parsed?.testimonials) ? parsed!.testimonials! : defaultTestimonials,
    settings: Array.isArray(parsed?.settings) ? parsed!.settings! : defaultSettings,
    bake_of_week: Array.isArray(parsed?.bake_of_week) ? parsed!.bake_of_week! : [],
    cake_menu_items: Array.isArray(parsed?.cake_menu_items) ? parsed!.cake_menu_items! : defaultCakeMenuItems,
  };
}

function readDB(): DatabaseSchema {
  try {
    if (!fs.existsSync(DATA_DIR)) {
      fs.mkdirSync(DATA_DIR, { recursive: true });
    }
    if (!fs.existsSync(DB_FILE)) {
      const initialData = normalise(null);
      fs.writeFileSync(DB_FILE, JSON.stringify(initialData, null, 2), 'utf-8');
      return initialData;
    }
    // Retry a few times â€” another process may be mid-write (rename is atomic
    // on the same volume, but Windows AV/indexers can briefly lock the file).
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        const content = fs.readFileSync(DB_FILE, 'utf-8');
        return normalise(JSON.parse(content));
      } catch (err) {
        if (attempt === 2) throw err;
        sleepSync(25 * (attempt + 1));
      }
    }
    return normalise(null); // unreachable, satisfies TS
  } catch (err) {
    // The file is unreadable/corrupted â€” back it up so data can be inspected,
    // then fall back to defaults instead of crashing every request.
    console.error('Database file unreadable, backing up and resetting:', err);
    try {
      if (fs.existsSync(DB_FILE)) {
        fs.copyFileSync(DB_FILE, `${DB_FILE}.corrupt-${Date.now()}`);
      }
    } catch { /* best effort */ }
    return normalise(null);
  }
}

function writeDB(data: DatabaseSchema): boolean {
  // Atomic write: dump to a temp file first, then rename over the real file.
  // A reader can therefore never observe a half-written JSON document.
  try {
    if (!fs.existsSync(DATA_DIR)) {
      fs.mkdirSync(DATA_DIR, { recursive: true });
    }
    fs.writeFileSync(TMP_FILE, JSON.stringify(data, null, 2), 'utf-8');
    fs.renameSync(TMP_FILE, DB_FILE);
    return true;
  } catch (err) {
    console.error('Atomic write failed, falling back to direct write:', err);
    try {
      fs.writeFileSync(DB_FILE, JSON.stringify(data, null, 2), 'utf-8');
      return true;
    } catch (fallbackErr) {
      console.error('Failed to write database file:', fallbackErr);
      return false;
    }
  }
}


// ----- Bake of Week -----
export async function getDbBakeOfWeek(): Promise<BakeOfWeek[]> {
  return enqueue(() => {
    const db = readDB();
    return [...db.bake_of_week].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
  });
}

export async function getDbBakeOfWeekById(id: string): Promise<BakeOfWeek | null> {
  return enqueue(() => {
    const db = readDB();
    return db.bake_of_week.find((b) => String(b.id) === String(id)) || null;
  });
}

export async function createDbBakeOfWeek(data: Partial<BakeOfWeek>): Promise<BakeOfWeek> {
  return enqueue(() => {
    const db = readDB();
    if (data.active) {
      db.bake_of_week.forEach((b) => {
        b.active = false;
      });
    }
    const newItem: BakeOfWeek = {
      id: data.id || `bow-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
      product_id: data.product_id || undefined,
      title: data.title || 'Bake of the Week',
      subtitle: data.subtitle || 'Bake of the Week',
      price: Number(data.price) || 0,
      unit: data.unit || '/whole',
      image: data.image || '',
      secondary_image: data.secondary_image !== undefined ? data.secondary_image : 'https://images.unsplash.com/photo-1623334044303-241021148842?w=600&auto=format&fit=crop&q=80',
      description: data.description || '',
      active: data.active !== false,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
    db.bake_of_week.push(newItem);
    writeDB(db);
    return newItem;
  });
}

export async function updateDbBakeOfWeek(id: string, updates: Partial<BakeOfWeek>): Promise<BakeOfWeek | null> {
  return enqueue(() => {
    const db = readDB();
    const idx = db.bake_of_week.findIndex((b) => String(b.id) === String(id));
    if (idx === -1) return null;
    if (updates.active) {
      db.bake_of_week.forEach((b) => {
        if (String(b.id) !== String(id)) b.active = false;
      });
    }
    db.bake_of_week[idx] = {
      ...db.bake_of_week[idx],
      ...updates,
      updated_at: new Date().toISOString(),
    };
    writeDB(db);
    return db.bake_of_week[idx];
  });
}

export async function deleteDbBakeOfWeek(id: string): Promise<boolean> {
  return enqueue(() => {
    const db = readDB();
    const initialLen = db.bake_of_week.length;
    db.bake_of_week = db.bake_of_week.filter((b) => String(b.id) !== String(id));
    if (db.bake_of_week.length !== initialLen) {
      writeDB(db);
      return true;
    }
    return false;
  });
}


// ----- Products -----
export async function getDbProducts(): Promise<MenuItem[]> {
  return enqueue(() => {
    const db = readDB();
    return db.products.sort((a, b) => (a.display_order || 0) - (b.display_order || 0));
  });
}

export async function getDbProductById(id: string): Promise<MenuItem | null> {
  return enqueue(() => {
    const db = readDB();
    return db.products.find((p) => String(p.id) === String(id)) || null;
  });
}

function normaliseImages(images: unknown): string[] | undefined {
  if (!Array.isArray(images)) return undefined;
  return images.map((i) => String(i).trim()).filter(Boolean);
}

export async function createDbProduct(data: Partial<MenuItem>): Promise<MenuItem> {
  return enqueue(() => {
    const db = readDB();
    const images = normaliseImages(data.images);
    const fallbackImage = 'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=600&h=500&fit=crop';
    const newProduct: MenuItem = {
      id: data.id || `p-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
      name: data.name || 'Untitled Product',
      description: data.description || '',
      price: Number(data.price) || 0,
      unit: data.unit || '/piece',
      category: data.category || 'pastry',
      badge: data.badge || undefined,
      image: data.image || images?.[0] || fallbackImage,
      images: images?.length ? images : (data.image ? [data.image] : undefined),
      rating: Number(data.rating) || 5,
      featured: Boolean(data.featured),
      is_bake_of_week: Boolean(data.is_bake_of_week),
      display_order: Number(data.display_order) || db.products.length + 1,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
    db.products.push(newProduct);
    writeDB(db);
    return newProduct;
  });
}

export async function updateDbProduct(id: string, updates: Partial<MenuItem>): Promise<MenuItem | null> {
  return enqueue(() => {
    const db = readDB();
    const idx = db.products.findIndex((p) => String(p.id) === String(id));
    if (idx === -1) return null;
    const images = normaliseImages(updates.images);
    const merged: Partial<MenuItem> = { ...updates };
    if (images) {
      merged.images = images;
      if (!updates.image) {
        merged.image = images[0];
      }
    }
    db.products[idx] = {
      ...db.products[idx],
      ...merged,
      updated_at: new Date().toISOString(),
    };
    writeDB(db);
    return db.products[idx];
  });
}

export async function deleteDbProduct(id: string): Promise<boolean> {
  return enqueue(() => {
    const db = readDB();
    const initialLen = db.products.length;
    db.products = db.products.filter((p) => String(p.id) !== String(id));
    if (db.products.length !== initialLen) {
      writeDB(db);
      return true;
    }
    return false;
  });
}

// ----- Orders -----
export async function getDbOrders(): Promise<Order[]> {
  return enqueue(() => {
    const db = readDB();
    return db.orders.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
  });
}

export async function getDbOrderById(id: string): Promise<Order | null> {
  return enqueue(() => {
    const db = readDB();
    return db.orders.find((o) => String(o.id) === String(id)) || null;
  });
}

export async function createDbOrder(data: Partial<Order> & { payment_method?: string }): Promise<Order> {
  return enqueue(() => {
    const db = readDB();
    const newOrder: Order = {
      id: typeof data.id === 'string' && data.id.trim() ? data.id.trim().slice(0, 32) : `ord-${Date.now().toString().slice(-5)}${Math.random().toString(36).slice(2, 4)}`,
      customer_name: data.customer_name || 'Guest Customer',
      customer_phone: data.customer_phone || '',
      customer_email: data.customer_email || '',
      customer_address: data.customer_address || '',
      items: Array.isArray(data.items) ? data.items : [],
      total: Number(data.total) || 0,
      payment_method: data.payment_method || 'Cash on Delivery',
      status: (data.status as Order['status']) || 'pending',
      notes: data.notes || '',
      variant: data.variant,
      message_on_item: data.message_on_item,
      item_note: data.item_note,
      delivery_date: data.delivery_date,
      delivery_time: data.delivery_time,
      delivery_location: data.delivery_location,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
    db.orders.unshift(newOrder);
    writeDB(db);
    return newOrder;
  });
}

export async function updateDbOrder(id: string, updates: Partial<Order>): Promise<Order | null> {
  return enqueue(() => {
    const db = readDB();
    const idx = db.orders.findIndex((o) => String(o.id) === String(id));
    if (idx === -1) return null;
    db.orders[idx] = {
      ...db.orders[idx],
      ...updates,
      updated_at: new Date().toISOString(),
    };
    writeDB(db);
    return db.orders[idx];
  });
}

export async function deleteDbOrder(id: string): Promise<boolean> {
  return enqueue(() => {
    const db = readDB();
    const initialLen = db.orders.length;
    db.orders = db.orders.filter((o) => String(o.id) !== String(id));
    if (db.orders.length !== initialLen) {
      writeDB(db);
      return true;
    }
    return false;
  });
}

// ----- Testimonials -----
export async function getDbTestimonials(): Promise<Testimonial[]> {
  return enqueue(() => {
    const db = readDB();
    return db.testimonials.sort((a, b) => new Date(b.created_at || '').getTime() - new Date(a.created_at || '').getTime());
  });
}

export async function createDbTestimonial(data: Partial<Testimonial>): Promise<Testimonial> {
  return enqueue(() => {
    const db = readDB();
    const newT: Testimonial = {
      id: data.id || `t-${Date.now()}`,
      name: data.name || 'Anonymous',
      initials: data.initials || data.name?.slice(0, 2).toUpperCase() || 'JB',
      role: data.role || 'Verified Customer',
      text: data.text || '',
      rating: Number(data.rating) || 5,
      approved: data.approved !== undefined ? data.approved : true,
      created_at: new Date().toISOString(),
    };
    db.testimonials.unshift(newT);
    writeDB(db);
    return newT;
  });
}

export async function updateDbTestimonial(id: string, updates: Partial<Testimonial>): Promise<Testimonial | null> {
  return enqueue(() => {
    const db = readDB();
    const idx = db.testimonials.findIndex((t) => String(t.id) === String(id));
    if (idx === -1) return null;
    db.testimonials[idx] = {
      ...db.testimonials[idx],
      ...updates,
    };
    writeDB(db);
    return db.testimonials[idx];
  });
}

export async function deleteDbTestimonial(id: string): Promise<boolean> {
  return enqueue(() => {
    const db = readDB();
    const initialLen = db.testimonials.length;
    db.testimonials = db.testimonials.filter((t) => String(t.id) !== String(id));
    if (db.testimonials.length !== initialLen) {
      writeDB(db);
      return true;
    }
    return false;
  });
}

// ----- Settings -----
export async function getDbSettings(): Promise<Setting[]> {
  return enqueue(() => {
    const db = readDB();
    return db.settings;
  });
}

export async function updateDbSettings(settingsToUpdate: Array<{ key: string; value: string }>): Promise<Setting[]> {
  return enqueue(() => {
    const db = readDB();
    for (const item of settingsToUpdate) {
      const idx = db.settings.findIndex((s) => s.key === item.key);
      if (idx !== -1) {
        db.settings[idx].value = item.value;
        db.settings[idx].updated_at = new Date().toISOString();
      } else {
        db.settings.push({
          key: item.key,
          value: item.value,
          updated_at: new Date().toISOString(),
        });
      }
    }
    writeDB(db);
    return db.settings;
  });
}

// ----- Cake Menu Items -----
export async function getDbCakeMenuItems(): Promise<CakeMenuItemRecord[]> {
  return enqueue(() => {
    const db = readDB();
    return [...(db.cake_menu_items || [])].sort((a, b) => (a.display_order ?? 0) - (b.display_order ?? 0));
  });
}

export async function getDbCakeMenuItemById(id: string): Promise<CakeMenuItemRecord | null> {
  return enqueue(() => {
    const db = readDB();
    const item = (db.cake_menu_items || []).find((c) => String(c.id) === String(id));
    return item ? { ...item } : null;
  });
}

export async function createDbCakeMenuItem(
  item: Omit<CakeMenuItemRecord, 'id' | 'created_at' | 'updated_at'>
): Promise<CakeMenuItemRecord> {
  return enqueue(() => {
    const db = readDB();
    if (!db.cake_menu_items) {
      db.cake_menu_items = [...defaultCakeMenuItems];
    }
    const maxOrder = db.cake_menu_items.reduce((max, c) => Math.max(max, c.display_order ?? 0), 0);
    const now = new Date().toISOString();
    const newItem: CakeMenuItemRecord = {
      id: `cake-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      name: item.name,
      nepali_subtitle: item.nepali_subtitle || '',
      price: Number(item.price),
      cat: item.cat || 'popular',
      badge: item.badge || '',
      desc: item.desc || '',
      tags: Array.isArray(item.tags) ? item.tags : [],
      image: item.image || '',
      weight: item.weight || '1 Pound',
      eggless: item.eggless !== undefined ? Boolean(item.eggless) : true,
      available: item.available !== undefined ? Boolean(item.available) : true,
      display_order: item.display_order !== undefined ? Number(item.display_order) : maxOrder + 1,
      created_at: now,
      updated_at: now,
    };
    db.cake_menu_items.push(newItem);
    writeDB(db);
    return newItem;
  });
}

export async function updateDbCakeMenuItem(
  id: string,
  updates: Partial<CakeMenuItemRecord>
): Promise<CakeMenuItemRecord | null> {
  return enqueue(() => {
    const db = readDB();
    if (!db.cake_menu_items) {
      db.cake_menu_items = [...defaultCakeMenuItems];
    }
    const idx = db.cake_menu_items.findIndex((c) => String(c.id) === String(id));
    if (idx === -1) return null;

    db.cake_menu_items[idx] = {
      ...db.cake_menu_items[idx],
      ...updates,
      price: updates.price !== undefined ? Number(updates.price) : db.cake_menu_items[idx].price,
      display_order: updates.display_order !== undefined ? Number(updates.display_order) : db.cake_menu_items[idx].display_order,
      updated_at: new Date().toISOString(),
    };
    writeDB(db);
    return db.cake_menu_items[idx];
  });
}

export async function deleteDbCakeMenuItem(id: string): Promise<boolean> {
  return enqueue(() => {
    const db = readDB();
    if (!db.cake_menu_items) {
      db.cake_menu_items = [...defaultCakeMenuItems];
    }
    const initialLen = db.cake_menu_items.length;
    db.cake_menu_items = db.cake_menu_items.filter((c) => String(c.id) !== String(id));
    if (db.cake_menu_items.length !== initialLen) {
      writeDB(db);
      return true;
    }
    return false;
  });
}
